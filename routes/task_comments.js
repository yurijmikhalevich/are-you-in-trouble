...
exports.retrieve = function(req) {
  ...
  checkAccess(user.id, user.role, req.data.taskId,
      cbs.doNext(req, function(res) {
        if (res === false) {
          req.io.emit('err', 'Unauthorized');
        } else {
          db.taskComments.retrieve(req.data.taskId,
              cbs.doNext(req, function(comments) {
      ...
      cbs.doNext(req, function(res) {
        if (res === false) {
          ...
          db.taskComments.save(req.data, cbs.doNext(req, function(comment) {
            ...
            req.io.room('task comments' + req.data.taskId).broadcast(
                event, comment);
...
exports.remove = function(req) {
  ...
  db.taskComments.remove(req.data.commentId,
      cbs.doNext(req, function(comment) {
        req.io.broadcast('task comments:remove',
            {commentId: comment.id, taskId: comment.taskId});
...
function checkAccess(userId, userRole, taskId, cb) {
  ...
  if (userRole === 'client') {
    checkAvailability = function(cb) {
      db.tasks.retrieve.forClient(userId, 0, 1, null, filters, cb);
    };
  } else if (userRole === 'helper') {
    checkAvailability = function(cb) {
      db.tasks.retrieve.forHelper(userId, 0, 1, null, filters, cb);
    };
  } else if (userRole === 'subdepartment chief') {
    checkAvailability = function(cb) {
      db.tasks.retrieve.forSubdepartmentChief(userId, 0, 1, null, filters, cb);
    };
  } else { // if (userRole === 'department chief'
    checkAvailability = function(cb) {
      cb(null, [{notice: 'chief department has access to all tasks'}]);
    };
  }
