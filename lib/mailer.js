...
exports.sendMail = function(to, subject, text) {
  ...
  logger.debug('Sending mail with subject %s and text %s to %s', subject, text,
      to);
  transport.sendMail({
...
exports.mailUsersAboutTaskUpdate = function(event, task) {
  ...
  } else if (['tasks:add helper', 'tasks:remove helper'].indexOf(event) !==
      -1) {
    subject = settings.mailer.blanks.helpersChanged.subject;
    text = settings.mailer.blanks.helpersChanged.text;
  }
  if (subject) { // && text
    db.tasks.retrieve.forDepartmentChief(0, 1,
        [{column: 'id', direction: 'ASC'}], {id: task.id},
        function(err, tasks) {
          if (err || !tasks.length) {
            logger.error(err.toString(), err);
          } else {
...
exports.mailUsersAboutTaskCommentUpdate = function(event, comment) {
  if (event === 'task comments:insert') {
    async.waterfall([
      function(cb) {
        db.tasks.retrieve.forDepartmentChief(0, 1,
            [{column: 'id', direction: 'ASC'}], {id: comment.taskId},
            function(err, tasks) {
              if (err || !tasks.length) {
 ...
 * @param {string} string
 * @param {Object} task
 * @param {Object=} opt_comment
 * @return {string}
 */
function compileString(string, task, opt_comment) {
  return string.replace(/{{id}}/g, task.id)
    ...
    .replace(/{{helpers}}/g,
          task.helpers.length ? task.helpers.join(', ') : 'Not assigned')
    .replace(/{{comment}}/g, opt_comment ? opt_comment.content : undefined)
    .replace(/{{author}}/g, opt_comment ? opt_comment.user : undefined);
}
...
function prepareTaskForMailing(task, cb) {
  async.parallel([
    function(cb) {
      db.registries.retrieve('university department',
          task.universityDepartmentId, cb);
    }, function(cb) {
      ...
      if (task.helperIds.length) {
        Array.prototype.push.apply(profileIds, task.helperIds);
      }
      if (profileIds.length) {
        db.profiles.retrieve(0, 100, [{column: 'id', direction: 'ASC'}],
            {id: profileIds}, cb);
      } else {
        cb(null, []);
      }
    ...
    if (err) {
      ...
      task.helpers = [];
      logger.debug('profiles', res[3]);
      res[3].forEach(function(profile) {
        if (profile.id === task.clientId) {
          task.client = profile.displayName;
        } else {
          task.helpers.push(profile.displayName);
        }
      });
      logger.debug('Prepared task', task);
      cb(null, task);
    }
  });
}
...
function prepareCommentForMailing(comment, cb) {
  db.profiles.retrieve(0, 1, [{column: 'id', direction: 'ASC'}],
      {id: comment.userId}, function(err, res) {
        if (err || !res.length) {
          cb(err || new errors.Internal(), null);
        } else {
          comment.user = res[0].displayName;
          logger.debug('Prepared comment', comment);
          cb(null, comment);
        }
      });
}
  ...
  async.parallel([
    ...
    if (err) {
      ...
      res.forEach(function(profiles) {
        Array.prototype.push.apply(profileList, profiles);
      });
      ...
      logger.debug('Mail list', mailList);
      cb(null, mailList);
    }
  });
}
