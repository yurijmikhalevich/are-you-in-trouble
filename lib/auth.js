...
/**
 * Accepts login, password, performs authentication and calls cb passing to it
...
exports.localStrategy = function(username, password, cb) {
  db.query('SELECT * FROM "' + prefix + 'user" WHERE username = $1',
        ...
        if (err || !res.rowCount) {
          ...
          if (!user.password || !user.password.length) {
            cb(new errors.Internal(
                'The user is not allowed to authenticate in this way'), null);
          } else {
            bcrypt.compare(password, user.password, function(err, res) {
              if (err || !res) {
                cb(err || new errors.Internal('Invalid username or password'),
                    null);
              } else {
                cb(null, user);
              }
...
exports.ldapStrategy = function(user, cb) {
  db.profiles.retrieve(0, 1, undefined, {email: user.mail},
      function(err, profiles) {
        if (err) {
          cb(err, null);
        } else {
          if (!profiles.length) {
            var universityDepartmentName =
                getUniversityDepartmentName(user.distinguishedName);
            if (!universityDepartmentName) {
              cb(new errors.Internal(
                  'Cannot create user with undefined university department'),
                  null);
            } else {
              async.waterfall([
                function(cb) {
                  db.registries.retrieve('university department',
                      universityDepartmentName, cb);
                }, function(departments, cb) {
                  if (departments.length) {
                    cb(null, departments[0]);
                  } else {
                    db.registries.save('university department',
                        {name: universityDepartmentName}, cb);
                  }
...
/**
 * Serialize user object into session object and calls cb passing to it error
 * object and session object
 * @param {Object} user
 * @param {Function} cb
 */
 ...
 * @param {String} username
 * @param {String} password
 * @param {String} email Email addresses, used as identifier with LDAP
 * authorization
 * @param {String} role
...
exports.createUser = function(username, password, email, role,
                              universityDepartmentId, subdepartmentId, cb) {
  if (universityDepartmentId) {
    // there is should be one - university department or subdepartment - defined
    subdepartmentId = null;
  }
  bcrypt.hash(password, 8, function(err, hash) {
    if (err) {
      ...
      db.query('INSERT INTO "' + prefix + 'user" (created_at, updated_at,' +
          ' username, password, email, role, university_department_id,' +
          ' subdepartment_id, display_name)' +
          ' VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING id',
          [new Date(), new Date(), username, hash, email, role,
            universityDepartmentId, subdepartmentId, username],
          function(err, res) {
            if (err || !res.rowCount) {
              cb(err || new errors.Internal(), null);
            } else {
