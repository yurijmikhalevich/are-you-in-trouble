  ...
  bcrypt.hash(password, 8, function (err, hash) {
    if (err) {
      ...
      db.query('INSERT INTO "' + prefix + 'user" (created_at, updated_at, username, password, email, role, ' +
        'university_department_id, subdepartment_id) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING id',
        [ new Date(), new Date(), username, hash, email, role, universityDepartmentId, subdepartmentId ],
        function (err, res) {
          if (err || !res.rowCount) {
            cb(err || new errors.Internal(), null);
          } else {
