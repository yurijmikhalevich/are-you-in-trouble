 ...
 * @param {Function} cb
 */
exports.createUser = function (username, password, email, role, universityDepartmentId, cb) {
  bcrypt.hash(password, 8, function (err, hash) {
    if (err) {
      cb(err, null);
    } else {
      db.query('INSERT INTO "' + prefix + 'user" (created_at, updated_at, username, password, email, role, ' +
        'university_department_id) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id',
        [ new Date(), new Date(), username, hash, email, role, universityDepartmentId ],
        function (err, res) {
          if (err || !res.rowCount) {
            cb(err || new errors.Internal(), null);
          } else {
