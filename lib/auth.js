...
exports.localStrategy = function (username, password, cb) {
  db.query('SELECT * FROM "' + prefix + 'user" WHERE username = $1', [ username ], function (err, res) {
    if (err || !res.rowCount) {
      cb(err || new errors.InternalError('Invalid username or password'), null);
    } else {
      var user = res.rows[0];
      if (!user.password || !user.password.length) {
        cb(new errors.InternalError('The user is not allowed to authenticate in this way'), null);
      } else {
...
exports.ldapStrategy = function (user, cb) {
  db.query('SELECT * FROM "' + prefix + 'user" WHERE email = $1', [ user.email ], function (err, res) {
    if (err) {
      ...
      if (!res.rowCount) {
        // TODO: create user
        cb(new errors.InternalError('User creation not implemented yet'), null);
      } else {
        cb(null, res.rows[0]);
      }
    }
