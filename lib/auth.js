  ...
  db.profiles.retrieve(0, 1, undefined, { email: user.mail }, function (err, profiles) {
    if (err) {
      ...
      if (!profiles.length) {
        async.waterfall([
          function (cb) {
            db.registries.retrieve('university_department', user.cn, cb);
          }, function (departments, cb) {
            if (departments.length) {
              cb(null, departments[0]);
            } else {
              db.registries.save('university_department', { name: user.cn }, cb);
            }
            ...
            db.profiles.save({
              ...
              phone: null,
              role: 'client',
              university_department_id: department.id,
  ...
  bcrypt.hash(password, 8, function (err, hash) {
    if (err) {
      ...
      db.query('INSERT INTO "' + prefix + 'user" (created_at, updated_at, username, password, email, role, ' +
        'university_department_id, subdepartment_id, displayname) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)' +
        'RETURNING id',
