...
module.exports = function(cb) {
  pool.query('SELECT relname FROM pg_class WHERE relname = $1',
      [prefix + 'user'], function(err, res) {
        if (err) {
          cb(err, null);
        } else {
          if (res.rowCount === 1) {
...
function initializeDatabase(cb) {
  var tableAndOtherBaseStuffQueries = [
    'CREATE OR REPLACE FUNCTION "' + prefix + 'fn_array_agg_notnull"' +
        ' (arr ANYARRAY, el ANYELEMENT)' +
        ' RETURNS ANYARRAY' +
        ' AS $$' +
        ' BEGIN' +
        ...
        // this needed because default ARRAY_AGG returns array with one NULL
    ...
    'CREATE TYPE "' + prefix + 'user_role" ' +
        'AS ENUM (\'client\', \'helper\', \'subdepartment chief\',' +
        ' \'department chief\');' +
        ...
        'email VARCHAR(255) UNIQUE NOT NULL,' + // this field is used for
        // associating system account with LDAP account
        'phone VARCHAR(15),' +
    ...
    'CREATE TABLE "' + prefix + 'university_department" (' +
        'id SERIAL PRIMARY KEY,' +
        'name VARCHAR(80) UNIQUE NOT NULL' + // this field is used for
        // associating system department with LDA department
        ');',
  ...
  var constraintsQueries = [
    'ALTER TABLE "' + prefix + 'user" ADD CONSTRAINT' +
        ' user2university_department ' +
        'FOREIGN KEY (university_department_id) REFERENCES "' + prefix +
        'university_department" (id);' +
        'ALTER TABLE "' + prefix + 'user" ADD CONSTRAINT user2subdepartment' +
        ' FOREIGN KEY (subdepartment_id) ' +
        'REFERENCES "' + prefix + 'subdepartment" (id);',
    'ALTER TABLE "' + prefix + 'task" ADD CONSTRAINT task2type FOREIGN KEY' +
        ' (type_id) ' +
        'REFERENCES "' + prefix + 'task_type" (id);' +
        'ALTER TABLE "' + prefix + 'task" ADD CONSTRAINT task2client' +
        ' FOREIGN KEY (client_id) ' +
        'REFERENCES "' + prefix + 'user" (id) ON DELETE CASCADE;' +
        'ALTER TABLE "' + prefix + 'task" ADD CONSTRAINT' +
        ' task2university_department ' +
        'FOREIGN KEY (university_department_id) REFERENCES "' + prefix +
        'university_department" (id);' +
        'ALTER TABLE "' + prefix + 'task" ADD CONSTRAINT task2subdepartment' +
        ' FOREIGN KEY (subdepartment_id) ' +
        'REFERENCES "' + prefix + 'subdepartment" (id);',
    'ALTER TABLE "' + prefix + 'task_type" ADD CONSTRAINT' +
        ' task_type2subdepartment FOREIGN KEY (subdepartment_id) ' +
        'REFERENCES "' + prefix + 'subdepartment" (id);',
    'CREATE UNIQUE INDEX ON "' + prefix + 'task2helper" (task_id, helper_id);',
    'ALTER TABLE "' + prefix + 'task2helper" ADD CONSTRAINT task2helper2task' +
        ' FOREIGN KEY (task_id) ' +
        'REFERENCES "' + prefix + 'task" (id) ON DELETE CASCADE;' +
        'ALTER TABLE "' + prefix + 'task2helper" ADD CONSTRAINT' +
        ' task2helper2user FOREIGN KEY (helper_id) ' +
        'REFERENCES "' + prefix + 'user" (id) ON DELETE CASCADE;',
    'ALTER TABLE "' + prefix + 'task_comment" ADD CONSTRAINT' +
        ' task_comment2task FOREIGN KEY (task_id) ' +
        'REFERENCES "' + prefix + 'task" (id) ON DELETE CASCADE;' +
        'ALTER TABLE "' + prefix + 'task_comment" ADD CONSTRAINT' +
        ' task_comment2user FOREIGN KEY (user_id) ' +
        'REFERENCES "' + prefix + 'user" (id) ON DELETE CASCADE;'
  ];
  async.series([
    function(cb) {
      async.map(tableAndOtherBaseStuffQueries,
          function(query, cb) { pool.query(query, cb); }, cb);
    }, function(cb) {
      async.map(constraintsQueries,
          function(query, cb) { pool.query(query, cb); }, cb);
    }, function(cb) {
      auth.createUser('kramer.a', '123', 'kramer.a@kubsau.ru',
          'department chief', null, null, cb);
    }
...
function fillDatabaseWithTestData(cb) {
  var subdepartmentAndUniversityDepartmentQueries = [
    'INSERT INTO "' + prefix + 'university_department" (name) VALUES' +
        ' (\'ФПИ\'), (\'КСТ\'), (\'ТССА\'), (\'ТХПЖП\'),' +
        ' (\'ТХПРП\'), (\'БББ\') RETURNING id',
    'INSERT INTO "' + prefix + 'subdepartment" (name) VALUES' +
        ' (\'Отдел по поддержке прикладного ПО\'),' +
        ' (\'Отдел по поддержке железа\'), (\'Отдел по поддержке ЛВС\')' +
        ' RETURNING id'
  ];
  async.map(subdepartmentAndUniversityDepartmentQueries,
      function(query, cb) { pool.query(query, cb); },
      function(err, ids) {
        if (err) {
          cb(err, null);
        } else {
          var userData = [
            {username: 'subdep1chief', password: '123',
              email: 'subdep1@kubsau.ru', role: 'subdepartment chief',
              subdepartmentId: ids[1].rows[0].id},
            {username: 'subdep2chief', password: '123',
              email: 'subdep2@kubsau.ru', role: 'subdepartment chief',
              subdepartmentId: ids[1].rows[1].id},
            {username: 'subdep3chief', password: '123',
              email: 'subdep3@kubsau.ru', role: 'subdepartment chief',
              subdepartmentId: ids[1].rows[2].id},
            {username: 'helper1', password: '123', email: 'helper1@kubsau.ru',
              role: 'helper', subdepartmentId: ids[1].rows[
                  Math.round(Math.random() * (ids[1].rows.length - 1))].id},
            {username: 'helper2', password: '123', email: 'helper2@kubsau.ru',
              role: 'helper', subdepartmentId: ids[1].rows[
                  Math.round(Math.random() * (ids[1].rows.length - 1))].id},
            {username: 'helper3', password: '123', email: 'helper3@kubsau.ru',
              role: 'helper', subdepartmentId: ids[1].rows[
                  Math.round(Math.random() * (ids[1].rows.length - 1))].id},
            {username: 'helper4', password: '123', email: 'helper4@kubsau.ru',
              role: 'helper', subdepartmentId: ids[1].rows[
                  Math.round(Math.random() * (ids[1].rows.length - 1))].id},
            {username: 'helper5', password: '123', email: 'helper5@kubsau.ru',
              role: 'helper', subdepartmentId: ids[1].rows[
                  Math.round(Math.random() * (ids[1].rows.length - 1))].id},
            {username: 'helper6', password: '123', email: 'helper6@kubsau.ru',
              role: 'helper', subdepartmentId: ids[1].rows[
                  Math.round(Math.random() * (ids[1].rows.length - 1))].id},
            {username: 'helper7', password: '123', email: 'helper7@kubsau.ru',
              role: 'helper', subdepartmentId: ids[1].rows[
                  Math.round(Math.random() * (ids[1].rows.length - 1))].id},
          ...
          var taskTypeQuery =
              'INSERT INTO "' + prefix + 'task_type" (name, subdepartment_id)' +
                  ' VALUES (\'Проблемы с сетью\', ' + ids[1].rows[2].id +
                  '), (\'Проблемы с компьютером\', ' + ids[1].rows[1].id +
                  '), ' + '(\'Проблемы с программами\', ' + ids[1].rows[0].id +
                  '), (\'Другие проблемы\', NULL) RETURNING id';
          async.parallel([
              ...
              var tasks = [
                {
                  ...
                  typeId: res[0].rows[
                      Math.round(Math.random() * (res[0].rows.length - 1))].id,
                  clientId: Math.round(Math.random()) ? res[1][
                      Math.round(Math.random() * 14) + 11].id : null,
                  universityDepartmentId: ids[0].rows[
                      Math.round(Math.random() * (ids[0].rows.length - 1))].id,
                  subdepartmentId: ids[1].rows[
                      Math.round(Math.random() * (ids[1].rows.length - 1))].id
                },
                {
                  content: 'Всё не так уж плохо',
                  typeId: res[0].rows[Math.round(Math.random() * (res[0].rows.length - 1))].id,
                  clientId: Math.round(Math.random()) ? res[1][Math.round(Math.random() * 14) + 11].id : null,
                  universityDepartmentId: ids[0].rows[Math.round(Math.random() * (ids[0].rows.length - 1))].id,
                  subdepartmentId: ids[1].rows[Math.round(Math.random() * (ids[1].rows.length - 1))].id
                },
                {
              ...
              async.map(tasks, db.tasks.save, function(err, taskIds) {
                if (err) {
                  ...
                  var task2helperQuery =
                      'INSERT INTO "' + prefix + 'task2helper"' +
                          ' (helper_id, task_id) VALUES (' +
                          res[1][Math.round(Math.random() * 7) + 3].id + ', ' +
                          taskIds[Math.round(Math.random() *
                              (taskIds.length - 1))].id + '), ' + '(' +
                          res[1][Math.round(Math.random() * 7) + 3].id + ', ' +
                          taskIds[Math.round(Math.random() *
                              (taskIds.length - 1))].id + '), ' + '(' +
...
function registerUser(userData, cb) {
  auth.createUser(userData.username, userData.password, userData.email,
      userData.role, userData.universityDepartmentId, userData.subdepartmentId,
      cb);
}
