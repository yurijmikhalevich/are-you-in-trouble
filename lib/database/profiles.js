...
exports.save = function (profile, cb) {
  if (profile.id) {
    pool.query('UPDATE "' + prefix + 'user" SET updated_at = $1, display_name = $2, phone = $3 WHERE id = $4 ' +
      'RETURNING *', [ new Date(), profile.displayName, profile.phone, profile.id ],
    ...
    pool.query('INSERT INTO "' + prefix + 'user" (created_at, updated_at, display_name, email, phone, role,' +
      ...
      new Date(), profile.displayName, profile.email, profile.phone, profile.role, profile.universityDepartmentId,
...
exports.setSubdepartment = function (userId, subdepartmentId, cb) {
  pool.query('UPDATE "' + prefix + 'user" SET subdepartment_id = $1 WHERE id = $2 RETURNING *',
    [ subdepartmentId, userId ], routines.returnOneEntity(cb));
};

exports.setUniversityDepartment = function (userId, universityDepartmentId, cb) {
  pool.query('UPDATE "' + prefix + 'user" SET university_department_id = $1 WHERE id = $2 RETURNING *',
...
exports.makeClient = function (userId, universityDepartmentId, cb) {
  pool.query('UPDATE "' + prefix + 'user" SET role = \'client\', university_department_id = $1 WHERE id = $2 ' +
    'RETURNING id', [ universityDepartmentId, userId ], routines.returnOneEntity(cb));
};
...
exports.makeDepartmentChief = function (userId, cb) {
  pool.query('UPDATE "' + prefix + 'user" SET role = \'department chief\' WHERE id = $1 RETURNING *',
    [ userId ], routines.returnOneEntity(cb));
};