...
//  async.waterfall([
//    function (cb) {
//      pool.query('SELECT subdepartment_id FROM "' + prefix + 'user" WHERE id = $1', [ helperId ], cb);
//    }, function (res, cb) {
//      if (!res.rowCount) {
...
//  async.waterfall([
//    function (cb) {
//      pool.query('SELECT subdepartment_id FROM "' + prefix + 'task" WHERE id = $1', [ taskId ], cb);
//    }, function (res, cb) {
//      if (!res.rowCount) {
//        cb(new errors.Internal('There is no task with id ' + taskId), null);
//      } else if (res.rows[0].subdepartment_id === null) {
//        cb(null, true);
//      } else {
//        var taskSubdepartmentId = res.rows[0].subdepartment_id;
//        pool.query('SELECT subdepartment_id FROM "' + prefix + 'user" WHERE id = $1', [ helperId ],
//          function (err, res) {
//            if (err || !res.rowCount) {
//              cb(err || new errors.Internal('There is no helper with id ' + helperId), null);
//            } else {
//              cb(null, res.rows[0].subdepartment_id === res.);
//            }
//          });
//      }
//    }
...
//  });
  /**
   * FIXME: allow adding only helpers from task's subdepartment (and automatically assign subdepartment to task,
   * if it is missing
   */
