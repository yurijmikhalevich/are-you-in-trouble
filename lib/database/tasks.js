...
var settings = require('cat-settings')
  ...
  , defaultOrder = [
    ...
    baseSelect: 'SELECT t.*, t.created_at::TEXT, t.updated_at::TEXT, ' +
      '"' + prefix + 'array_agg_notnull"(DISTINCT th.helper_id) AS helper_ids,' +
...
exports.save = function (task, cb) {
  if (task.id) {
    pool.query('UPDATE "' + prefix + 'task" SET updated_at = $1, content = $2, type_id = $3, client_id = $4, ' +
      'university_department_id = $5, subdepartment_id = $6 WHERE id = $7 RETURNING *, created_at::TEXT, ' +
      'updated_at::TEXT', [ new Date(), task.content, task.typeId, task.clientId, task.universityDepartmentId,
      task.subdepartmentId, task.id ], routines.returnOneEntity(cb, 'There is no task with id ' + task.id));
  } else {
    pool.query('INSERT INTO "' + prefix + 'task" (created_at, updated_at, content, type_id, client_id, ' +
      'university_department_id, subdepartment_id) VALUES ($1, $2, $3, $4, $5, $6, $7) ' +
      'RETURNING *, created_at::TEXT, updated_at::TEXT', [ new Date(), new Date(), task.content, task.typeId,
      task.clientId, task.universityDepartmentId, task.subdepartmentId ], routines.returnOneEntity(cb));
  }
};

exports.close = function (taskId, closedById, cb) {
  pool.query('UPDATE "' + prefix + 'task" SET closed_by_id = $1, updated_at = $2 WHERE id = $3 ' +
    'RETURNING *, created_at::TEXT, updated_at::TEXT', [ closedById, new Date(), taskId ],
    routines.returnOneEntity(cb, 'There is no task with id ' + taskId));
};

exports.remove = function (taskId, cb) {
  pool.query('DELETE FROM "' + prefix + 'task" WHERE id = $1 RETURNING *, created_at::TEXT, updated_at::TEXT',
    [ taskId ], routines.returnOneEntity(cb, 'There is no task with id ' + taskId));
};
