...
var settings = require('cat-settings')
  ...
  , defaultOrder = [
    ...
    baseSelect: 'SELECT t.*, "' + prefix + 'array_agg_notnull"(th.helper_id) AS helper_ids, COUNT(tc.id)::INT ' +
      'AS comment_count FROM "' + prefix + 'task" AS t',
...
exports.retrieve.forClient = function (clientId, offset, limit, order, filters, cb) {
  ...
  routines.execQuery(query, placeholdersData, 't', offset, limit, order || defaultOrder, filters || defaultFilters, cb);
};
...
exports.retrieve.forDepartmentChief = function (offset, limit, order, filters, cb) {
  ...
  routines.execQuery(query, [], 't', offset, limit, order || defaultOrder, filters || defaultFilters, cb);
};
