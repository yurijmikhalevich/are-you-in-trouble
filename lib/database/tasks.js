...
exports.forUniversityDepartment = function (universityDepartmentId, offset, limit, callback) {
  models.knex('university_department').where({ parent_id: universityDepartmentId }).select('id')
    .exec(function (err, universityDepartments) {
      if (err) {
        ...
        models.knex('task').whereIn('university_department_id', array).offset(offset).limit(limit).select()
          .exec(callback);
      }
    });
};